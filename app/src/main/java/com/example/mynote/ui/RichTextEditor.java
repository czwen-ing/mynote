...
public class RichTextEditor extends ScrollView {
    ...
    private int noteId = -1;//便签在数据库的ID，如果为新建便签则Id默认为-1
	...
	public RichTextEditor(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		firstEdit = createEditText("",
				dip2px(EDIT_FIRST_PADDING_TOP));
		firstEdit.setMinLines(14);
	...
	private void onImageCloseClick(View view) {
		if (!mTransitioner.isRunning()) {
			...
			mergeEditText();
            if (noteId != -1){
                DataUtils.removeImageFromDB(getContext().getContentResolver(),noteId,disappearingImageIndex);
            }
		}
	}
	...
	private EditText createEditText(String hint, int paddingTop) {
		final EditText editText = (EditText) inflater.inflate(R.layout.edit_item1,
				null);
		...
		editText.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
				...
				if (mOnTextChangeListener != null){
					mOnTextChangeListener.onTextChange(textCount);
				}
			}
			@Override
	...
	public void insertImage(Bitmap bitmap, String imagePath) {
		...
		hideKeyBoard();
		if (noteId != -1){
			DataUtils.addImageToDB(getContext().getContentResolver(),noteId,lastEditIndex + 1,imagePath
					,ImageUtils.bmpToByte(bitmap));
		}
	}
	...
	public void addImageViewAtIndex(final int index, Bitmap bmp,
		...
		Log.e("TAG","addImageViewAtIndex" + bmp.getHeight());

		// 调整imageView的高度

		int imageHeight = getWidth()  * bmp.getHeight() / bmp.getWidth();
		if (imageHeight == 0){
			imageHeight = 1020  * bmp.getHeight() / bmp.getWidth();
			Log.e("TAG","addImageViewAtIndex      getWidth" + getWidth());
		}
		Log.e("TAG","addImageViewAtIndex      imageHeight" + imageHeight);
	...
	public List<EditData> buildEditData() {
		...
		for (int index = 0; index < num; index++) {
			...
			if (itemView instanceof EditText) {
				...
				Log.e("TAG","buildEditData  imageIndex" + index);
	...
	OnTextChangeListener mOnTextChangeListener = null;
	...
	public int getTextCount() {
		...
		for (int index = 0; index < num; index++) {
			...
			if (itemView instanceof EditText) {
				EditText item = (EditText) itemView;
				int count = item.getText().toString().length();
	...
	public Bitmap getScaledBitmap(String filePath, int width) {
		Log.e("TAG","getScaledBitmap" + filePath);
